...
def mat2euler(M, cy_thresh=None, seq='zyx'):
    ...
    if cy_thresh is None:
        cy_thresh = torch.finfo(M.dtype).eps * 4
    ...
    return torch.stack([z, y, x], dim=0).reshape(B,3)
...
def pose_mat2vec(mat, seq='zyx'):
    """
    Convert transforamtion matrix to 6DoF parameters.
    Args:
        mat (tensor): a transformation matrix -- [B, 3, 4]
    Returns:
        vec: 6DoF parameters in the order of tx, ty, tz, rx, ry, rz -- [B, 6]
    """
    rot = mat[:, :3, :3]
    ...
    tra = mat[:, 3, :3]
    vec = torch.cat([tra, rot_vec], dim=1) #[B, 6]
    return vec
    


def inverse_warp(img, depth, ref_depth, pose, intrinsics, mode='bilinear', padding_mode='zeros'):
    """
    ...
    # T = pose_vec2mat(pose)  # [B,3,4] for pose_net
    T = pose[:, :3, :] # [B,3,4] for SLAM
    ...
    projected_img = F.grid_sample(img, pix_coords, mode, padding_mode=padding_mode, align_corners=False)
    projected_depth = F.grid_sample(ref_depth, pix_coords, mode, padding_mode=padding_mode, align_corners=False)
