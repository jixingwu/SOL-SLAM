...
def crawl_folders2(path, dataset='kitti'):
    if dataset == 'kitti':
        imgs_path, depths_path = path/'color', path/'depth'
        image_files = sorted(sum([(imgs_path).files('*.{}'.format(ext)) 
                                for ext in ['jpg', 'png']], []))
        depth_files = sorted(sum([(depths_path).files('*.{}'.format(ext)) 
                              for ext in ['jpg', 'png']], []))
    return image_files, depth_files
    
class ValidationFloder(data.Dataset):

    def __init__(self, root, transform=None, dataset='nyu'):
        self.root = Path(root)
        scene_list_path = self.root/'val'
        self.transform = transform
        self.dataset = dataset
        self.imgs, self.depth = crawl_folders2(scene_list_path, self.dataset)
    ...
    def __getitem__(self, index):
        ...
        if self.dataset in ['bonn', 'tum']:
            ...
            depth = torch.from_numpy(load_sparse_depth(
                self.depth[index]).astype(np.float32))
        elif self.dataset in ['kitti']:
            depth = torch.from_numpy(
                imread(self.depth[index]).astype(np.float32)).float()
