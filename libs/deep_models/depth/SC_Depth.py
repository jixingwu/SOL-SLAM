...
from libs.deep_models.depth.models.DepthNet import DepthNet
from .visualization import *

class SC_Depth(LightningModule):
    def __init__(self, hparams, lora_r, lora_r2):
        super(SC_Depth, self).__init__()
        self.save_hyperparameters()

        self.hparams.hparams.lora_r = lora_r
        self.hparams.hparams.lora_r2 = lora_r2

        # model
        self.depth_net = DepthNet(self.hparams.hparams.lora_r, 
                                  self.hparams.hparams.lora_r2,
                                  self.hparams.hparams.resnet_layers)
    ...
    def training_step(self, batch):
        ...
        # fixed, 数据输入还是有问题
        tgt_img, ref_imgs, pose, intrinsics = batch # tgt_img, ref_imgs, pose, intrinsics = batch
        ...
        poses = [pose]
        poses_inv = [torch.inverse(pose)]

        # poses = [self.pose_net(tgt_img, im) for im in ref_imgs] #  6DoF pose parameters from target to source [1,6]
